...
function equip_enka_change_load_character(input_obj) {
    equip_character_load(input_obj.party_id, enka_objects.characters[input_obj.index]);
}

function equip_enka_change_save_character(index) {
    var character = structuredClone(enka_objects.characters[index]);
    if (equip_character_storage_save_character(character)) {
        equip_character_storage_save_last();
    }    
}

function equip_enka_change_save_character_all() {
    var saved_characters = 0;
    for (var i = 0; i < enka_objects.characters.length; i++) {
        var character = structuredClone(enka_objects.characters[i]);
        if (character.id && equip_character_storage_save_character(character, false)) {
            saved_characters++;
        }
    }
        ...
        if (response.status == 200) {
            try {
                if (response.response) {
                    ...
                    equip_enka_update_data(response_obj.message, uid);
            ...
            try {
                ...
                utils_loading_show_error(response.status + " - " + err.error + ": " + err.message, "Enka Error occured:\n" + err.message)
            } catch (e) {
                utils_loading_show_error(e + ": " + response.responseText, "Unknown Enka Error occured:\n" + e)
            }
        }       
    } catch (e) {
...
function equip_enka_update_data(response, uid) {
    ...
    enka_objects.characters = characters;
    user_preferences.enka.last_uid = uid;
}
...
function equip_enka_display_all() {
    ...
    var enka_display_objects = [];

    for (var i = 0; i < enka_objects.characters.length; i++) {
        if (enka_objects.characters[i].id) {
            enka_display_objects.push(equip_enka_display(i));
        }       
    }
    enka_display_objects = utils_array_sort(enka_display_objects, "sort");
    for (var i = 0; i < enka_display_objects.length; i++) {
        parent.appendChild(enka_display_objects[i]);
    }
}

function equip_enka_display(index) {
    var enka_character = enka_objects.characters[index];
    ...
    name_row.appendChild(equip_character_storage_display_name(character, enka_character.name));
    ...
    obj.sort = character.name + " " + enka_character.name;
...
function equip_enka_return_character(uid, enka_char, custom_name = null) {
    ...
    while (skill_level.length > const_attack_level_types.length) {
        skill_level.shift();
    }
    for (var i = 0; i < const_attack_level_types.length; i++) {
        character["level" + const_attack_level_types[i]] = skill_level[i];
    }
    ...
    for (var i = 0; i < enka_char.equipList.length; i++) {
        ...
        if (enka_equip.flat.itemType == "ITEM_WEAPON") {
            ...
            artifact.main_stat = const_enka_stats_transform_table[enka_equip.flat.reliquaryMainstat.mainPropId];
            ...
            for (var ii = 0; ii < enka_equip.reliquary.appendPropIdList.length; ii++) {
                ...
                for (var iii = 0; iii < const_artifact_sub_stats; iii++) {
                    if (artifact.sub_stats[iii].id == const_artifact_sub_stats_options[0]) {
                        artifact.sub_stats[iii].id = sub.stat;
                        artifact.sub_stats[iii].value = sub.value;
                        filled_substat = true;
                        break;
                    } else if (artifact.sub_stats[iii].id == sub.stat) {
                        artifact.sub_stats[iii].value = utils_number_round(artifact.sub_stats[iii].value + sub.value, 3);
                        filled_substat = true;
                        break;
                    }
                }
    ...
    character.display_stats = equip_stats_return_display_stats(character);
    character.vision_stat = equip_stats_return_calculated_vision_stat(character.id, character.display_stats);
