...
function equip_effects_update_stats(party_id) {
    ...
    equip_skills_update_details_active_setup(party_id);
    ...
    for (var i = 0; i < user_objects.user_party[party_id].active_skills.length; i++) {
        ...
        if (skill_effects && skill_effects.length > 0) {
            ...
            output_party[party_id].skills.stats[i].initial.effects = structuredClone(skill_stats);
        }
    }
    equip_effects_update_infusion(party_id);
    equip_effects_update_special_value("conversion_special", party_id);
}
...
function equip_effects_update_infusion(party_id) {
    ...
    for (var i = 0; i < user_objects.user_party[party_id].active_skills.length; i++) {
        var skill_effects = user_objects.user_party[party_id].active_skills[i].effects;
        if (skill_effects && skill_effects.length > 0) {
            output_party[party_id].skills.output.initial.active.details[i].infusion = equip_effects_return_infusion(party_id, i);
        }
    }
}

function equip_effects_update_special_value(value_name, party_id) {
    output_party[party_id].effects[value_name] = equip_effects_return_special_value(value_name, party_id, null);

    for (var i = 0; i < user_objects.user_party[party_id].active_skills.length; i++) {
        var skill_effects = user_objects.user_party[party_id].active_skills[i].effects;
        if (skill_effects && skill_effects.length > 0) {
            output_party[party_id].skills.output.initial.active.details[i][value_name] = equip_effects_return_special_value(value_name, party_id, i);
        }
    }
}
...
function equip_effects_update_special_option(effect_list, apply, offset, source, source_party, party_id) {
    ...
    switch (apply.special) {
        ...
        case "nightsoul_1":
            ...
            var offset_ids = equip_effects_return_generic_offset(apply, offset, nightsoul, 1)
            new_effect.id = offset_ids.id;
            new_effect.max_id = offset_ids.max_id;
            break;
        case "moonsign":
        ...
        case "moonsign_1_2":
            ...
            if (moonsign > 0) {
                ...
                var offset_ids = equip_effects_return_generic_offset(apply, offset, moonsign, 1)
                new_effect.id = offset_ids.id;
                new_effect.max_id = offset_ids.max_id;
            }
        ...
        case "moonsign_1":
            var moonsign = equip_character_return_variable_count("moonsign");
            if (moonsign >= 1) {
                var offset_ids = equip_effects_return_generic_offset(apply, offset, 0, 0)
                new_effect.id = offset_ids.id;
                new_effect.max_id = offset_ids.max_id;
            }
            break;
        ...
        case "hexenzirkel_0_2":
            ...
            var offset_ids = equip_effects_return_generic_offset(apply, offset, hexenzirkel, 1)
            new_effect.id = offset_ids.id;
            new_effect.max_id = offset_ids.max_id;
        ...
        case "hexenzirkel_2_personal":
            ...
            if (hexenzirkel >= 2) {
                ...
                var offset_ids = equip_effects_return_generic_offset(apply, offset, hexenzirkel_personal, 1)
                new_effect.id = offset_ids.id;
                new_effect.max_id = offset_ids.max_id;
            }
        ...
        case "stellarconduct":
            var stellarconduct = equip_character_return_variable_count("stellarconduct");
            if (stellarconduct > 0) {
                var offset_ids = equip_effects_return_generic_offset(apply, offset, 0, 0)
                new_effect.id = offset_ids.id;
                new_effect.max_id = offset_ids.max_id;
        ...
        case "stellarswirl":
            var stellarswirl = equip_character_return_variable_count("stellarswirl");
            if (stellarswirl > 0) {
                var offset_ids = equip_effects_return_generic_offset(apply, offset, 0, 0)
                new_effect.id = offset_ids.id;
                new_effect.max_id = offset_ids.max_id;
        ...
        case "stellarglimmer":
            var stellarglimmer = equip_character_return_variable_count("stellarswirl") + equip_character_return_variable_count("stellarconduct");
            if (stellarglimmer > 0) {
                var offset_ids = equip_effects_return_generic_offset(apply, offset, 0, 0)
                new_effect.id = offset_ids.id;
                new_effect.max_id = offset_ids.max_id;
        ...
        case "stellarconduct_personal":
            var stellarconduct = equip_character_return_variable_count("stellarconduct");
            if (stellarconduct > 0 && data_characters[user_objects.user_party[party_id].id].radiance_stellarconduct) {
        ...
        case "same_region":
            var count = equip_character_return_variable_count("nation", data_characters[user_objects.user_party[party_id].id].nation);;
            if (count > 3) {
                count = 3;
            }
            var offset_ids = equip_effects_return_generic_offset(apply, offset, count-1, 2)
            new_effect.id = offset_ids.id;
            new_effect.max_id = offset_ids.max_id;
            break;

        case "traveler":
            if (user_objects.user_party[party_id].id.startsWith("traveler")) {
                var offset_ids = equip_effects_return_generic_offset(apply, offset, 0, 0)
                new_effect.id = offset_ids.id;
                new_effect.max_id = offset_ids.max_id;
            }
        ...
        case "geo_hydro_0_1_2_3":
            ...
            var offset_ids = equip_effects_return_generic_offset(apply, offset, geo_hydro_count, 3)
            new_effect.id = offset_ids.id;
            new_effect.max_id = offset_ids.max_id;
        ...
        case "hydro_0_1_2":
            ...
            var offset_ids = equip_effects_return_generic_offset(apply, offset, hydro_count, 2)
            new_effect.id = offset_ids.id;
            new_effect.max_id = offset_ids.max_id;
        ...
        case "same_element":
            ...
            var offset_ids = equip_effects_return_generic_offset(apply, offset, (same_element - 1), 3)
            new_effect.id = offset_ids.id;
            new_effect.max_id = offset_ids.max_id;
            break;
        ...
        case "different_element_0_1_2":
            ...
            var offset_ids = equip_effects_return_generic_offset(apply, offset, different_element, 2)
            new_effect.id = offset_ids.id;
            new_effect.max_id = offset_ids.max_id;
        ...
        case "natlan_different_element":
            ...
            if (count > 0) {
                var offset_ids = equip_effects_return_generic_offset(apply, offset, (count-1), 3)
                new_effect.id = offset_ids.id;
                new_effect.max_id = offset_ids.max_id;
            }
        ...
        case "sturm_und_drang":
            var pyro_count = equip_character_return_variable_count("vision", "pyro");
            var hydro_count = equip_character_return_variable_count("vision", "hydro");
            var electro_count = equip_character_return_variable_count("vision", "electro");
            var cryo_count = equip_character_return_variable_count("vision", "cryo");
            var max_count = Math.max(pyro_count, hydro_count, electro_count, cryo_count)

            if (max_count >= 1) {
                var vision_offset = 0;
                if (pyro_count >= 1 && pyro_count == max_count) {
                    vision_offset = 0;
                } else if (hydro_count >= 1 && hydro_count == max_count) {
                    vision_offset = 1;
                } else if (electro_count >= 1 && electro_count == max_count) {
                    vision_offset = 2;
                } else {
                    vision_offset = 3;
                }

                var offset_ids = equip_effects_return_generic_offset(apply, offset, vision_offset, 3)
                new_effect.id = offset_ids.id;
                new_effect.max_id = offset_ids.max_id;
            }
        ...
        case "dawn_wind_march":

            var stage = 0;
            if (equip_character_return_variable_count("vision", "anemo") >= 2) {
                stage += 1;
            }
            if (equip_character_return_variable_count("vision", "pyro") >= 2) {
                stage += 1;
            }
            if (equip_character_return_variable_count("vision", "hydro") >= 2) {
                stage += 1;
            }
            if (equip_character_return_variable_count("vision", "electro") >= 2) {
                stage += 1;
            }
            if (equip_character_return_variable_count("vision", "cryo") >= 2) {
                stage += 1;
            }
            if (stage > 2) {
                stage = 2;
            ...
            if (stage > 0) {
                var offset_ids = equip_effects_return_generic_offset(apply, offset, (stage - 1), 3)
                new_effect.id = offset_ids.id;
                new_effect.max_id = offset_ids.max_id;
            }
        ...
        case "snow_swan_dream":
            var active = false;
            var offset_val = 0;
            if (source_party == party_id) {
                active = true;
            } else if (user_objects.user_party[source_party].constel >= 4) {
                active = true;
                offset_val = 1;
            }

            if (active) {
                var offset_ids = equip_effects_return_generic_offset(apply, offset, offset_val, 1)
                new_effect.id = offset_ids.id;
                new_effect.max_id = offset_ids.max_id;
            }
        ...
        case "hunter_mark":
            var offset_val = 0;
            if (equip_character_return_variable_count("stellarconduct") >= 1) {
                offset_val += 1;
            }
            if (user_objects.user_party[source_party].constel >= 6) {
                offset_val += 2;
            }
            var offset_ids = equip_effects_return_generic_offset(apply, offset, offset_val, 3)
            new_effect.id = offset_ids.id;
            new_effect.max_id = offset_ids.max_id;
            break;
        ...
        case "const_1_2_6":
            var const_effect_ids = equip_effects_return_const_offset(source_party, apply, offset, [1, 2, 6]);
            new_effect.id = const_effect_ids.id;
            new_effect.max_id = const_effect_ids.max_id;
            break;
        ...
        case "energy_60_40":
            ...
            if (energy <= 60 ) {
                ...
                var offset_ids = equip_effects_return_generic_offset(apply, offset, energy_count, 1)
                new_effect.id = offset_ids.id;
                new_effect.max_id = offset_ids.max_id;
...
function equip_effects_return_effect_right(selected_option, source_party) {
    ...
    if (selected_option.conversion_special) {
        effect_right.appendChild(utils_create_statline(data_visions[selected_option.conversion_special].name + " Conversion for certain attacks", null));
    }
...
function equip_effects_return_stats_transformed(source_party_id, transform, input_value = null, skill_index = null, artifact_stat = null, artifact_stat_party = null) {
    ...
    if (skill_index === null) {
        ...
        var source_stats = output_party[source_party_id].skills.stats[skill_index];
    }
...
function equip_effects_return_special_value(value_name, party_id, skill_index = null) {
...
function equip_effects_return_const_offset(source_party, apply, offset, breakpoints) {
    ...
    var offset_val = 0;
    if (apply.offset) {
        var max_id = apply.offset.length * (breakpoints.length + 1) - 1;
        offset_val = apply.offset[offset];
    } else {
        var max_id = breakpoints.length;
    }
    ...
    return {
        ...
        "max_id": apply.id + max_id
    }
}

function equip_effects_return_generic_offset(apply, offset, current_version, max_version) {
    if (apply.offset) {
        var offset_val = apply.offset[offset] + current_version * apply.offset.length;
        var max_id = apply.offset.slice(-1)[0] + max_version * apply.offset.length;
    } else {
        var offset_val = current_version;
        var max_id = max_version;
    }
