...
if(isset($_SESSION["loggedin"]) && $_SESSION["loggedin"] === true){
	...
	if (strlen($input) > 10485760){
		...
		$sql = "UPDATE genshin_users SET user_preferences = ? WHERE id = ?";
		...
		if($input){
			if($stmt = mysqli_prepare($link, $sql)){
				...
				if (mysqli_stmt_execute($stmt)){
					...
					$errormessage = "Error when updating user preferences";
				}
			} else {		
				$errormessage = "Error when updating user preferences";
			}
			...
			$errorcode = 400;
			$errormessage = "No input user preferences provided";
		}
	}	
